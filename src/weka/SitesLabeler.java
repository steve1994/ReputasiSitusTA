...
public class SitesLabeler extends SitesMLProcessor{
    ...
    public static void main (String[] args){
        // Labeled sites dengan tipe reputasi 1
        int typeReputation = 1;
        ...
        for (int numSitesEachType=10;numSitesEachType<=100;numSitesEachType=numSitesEachType+10) {
            ...
            String fileName = "num_" + numSitesEachType + ".type_" + typeReputation + ".supervised.txt";
            String pathName = "database/weka/" + fileName;
            EksternalFile.saveInstanceWekaToExternalARFF(labeledSite.getSiteReputationRecord(),pathName);

            // Build and Evaluate Classifier (1 : NaiveBayes, 2 : ID3, 3 : J48)
            statisticEvaluationReport.append("\nNum Sites : " + numSitesEachType + "\n\n");
            for (int i=0;i<3;i++) {
                statisticEvaluationReport.append("\nAlgoritma Pembelajaran Supervised Learning ke-" + i+1 + " : \n\n");
                Classifier siteClassifier = labeledSite.buildLabelReputationModel(labeledSite.getSiteReputationRecord(), i+1);
                try {
                    // Full Training
